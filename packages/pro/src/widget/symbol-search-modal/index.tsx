// /klinecharts-workspace/packages/pro/src/widget/symbol-search-modal/index.tsx
...
const SymbolSearchModal: Component<SymbolSearchModalProps> = props => {
  ...
  // --- 关键修复：将回调模式的 datafeed 方法适配给 createResource ---
  const [symbolList] = createResource(value, async (userInput) => {
    // 创建一个 Promise，createResource 会等待它 resolve
    return new Promise<SymbolInfo[]>((resolve) => {
      // 调用 datafeed 的 searchSymbols 方法，并传入一个回调
      props.datafeed.searchSymbols(userInput, (symbols) => {
        // 当 datafeed 完成搜索后，通过 onResult 回调，我们 resolve 这个 Promise
        resolve(symbols);
      });
    });
  });
